...
func (e *UpdateExec) Next() (*Row, error) {
	...
	for _, entry := range row.RowKeys {
		...
		handle := entry.Handle
		if offset < 0 || offset + len(tbl.WritableCols()) > len(row.Data) {
			log.Errorf("index out of bound row data len:%d, writable cols len:%d, offset:%d",
				len(row.Data), len(tbl.WritableCols()), offset)
		}
